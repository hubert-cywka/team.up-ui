...
const SportEditTab = ({ mode, sportToEdit, onSuccess }: SportEditPanelProps) => {
  const {
    register,
    handleSubmit,
    getValues,
  ...
  const getSportNameInput = () => {
    return (
      <>
        <Input {...register('name')} className={styles.input} fullWidth />
  ...
  const getSelectSportAlert = () => {
    return <Alert message={`Select sport discipline you want to ${mode}.`} />;
  };

  const buildAlertWithConfirmButton = (variant: 'success' | 'error', text: string) => {
    return (
      <>
        <Alert message={text} variant={variant} />
